...
import { X, Code2, Image as ImageIcon, AlertCircle, Loader2, Layers } from "lucide-react";
...
const LanguageDialog: React.FC<Props> = ({ open, language, onClose, onSubmit }) => {
    const [form, setForm] = useState<LanguageRequest>({
        name: "",
        slug: "",
        iconUrl: "",
        isActive: true,
    ...
    useEffect(() => {
        if (open) {
            if (language) {
                ...
                setForm({ name: "", slug: "", iconUrl: "", isActive: true, frameworkIds: [] });
    ...
    const toggleFramework = (id: number) => {
        setForm(prev => {
            ...
            return {
                ...prev,
                frameworkIds: ids.includes(id) ? ids.filter(x => x !== id) : [...ids, id],
            };
        });
    };
            ...
            <div className="relative w-full max-w-md bg-white border border-ink-200 rounded-3xl shadow-soft-lg overflow-hidden animate-fade-in-up">
                ...
                <div className="px-5 py-4 max-h-[65vh] overflow-y-auto space-y-4">
                    ...
                    <div>
                        ...
                        <input
                            placeholder="vd: JavaScript, TypeScript, Python..."
                            value={form.name}
                    ...
                    <div>
                        ...
                        <input
                            placeholder="vd: javascript, typescript..."
                            value={form.slug}
                    ...
                    {availableFrameworks.length > 0 && (
                        <div className="pt-1">
                            <label className="text-xs font-semibold text-ink-700 mb-2 uppercase tracking-wide flex items-center gap-1.5">
                                <Layers size={11} /> Framework hỗ trợ
                            </label>
                            <div className="space-y-1.5 max-h-40 overflow-y-auto pr-0.5">
                                {availableFrameworks.map(fw => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className={`w-7 h-7 flex-shrink-0 rounded-lg flex items-center justify-center overflow-hidden border ${selected ? 'border-accent-200 bg-white' : 'border-ink-100 bg-ink-50'}`}>
                                                {fw.iconUrl
                                                    ? <img src={fw.iconUrl} alt={fw.name} className="w-full h-full object-cover" onError={e => { (e.target as HTMLImageElement).style.display='none'; }} />
                                                    : <span className="text-[9px] font-black text-ink-400">{fw.name.slice(0,2).toUpperCase()}</span>
                                                }
                                            </div>
                                            <span className={`text-sm font-semibold flex-1 truncate ${selected ? 'text-accent-700' : 'text-ink-700'}`}>{fw.name}</span>
                                            <div className={`w-4 h-4 rounded flex items-center justify-center flex-shrink-0 border transition-colors ${selected ? 'bg-accent-600 border-accent-600' : 'border-ink-300 bg-white'}`}>
                ...
                <div className="flex items-center gap-2 px-5 py-4 border-t border-ink-200 bg-ink-50/50">
                    ...
                    >
                        {loading ? (
                            ...
                            language ? "Cập nhật" : "Tạo ngôn ngữ"
                        )}
                    </button>
                </div>
            </div>
